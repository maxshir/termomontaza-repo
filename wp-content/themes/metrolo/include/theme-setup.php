...
function hoot_modify_read_more_link( $more = '[&hellip;]' ) {
	...
	$read_more = ( empty( $read_more ) ) ? sprintf( __( 'Read More %s', 'metrolo' ), '&rarr;' ) : $read_more;
	global $post;
	$read_more = '<a class="more-link" href="' . esc_url( get_permalink( $post->ID ) ) . '"><span>' . $read_more . '</span></a>';
