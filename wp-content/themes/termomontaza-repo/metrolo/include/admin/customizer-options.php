...
function hoot_theme_customizer_options() {
	...
	$settings['header_layout_help'] = array(
		'section'     => $section,
		'type'        => 'content',
		'content'     => '<em>' . __( 'Select options below to create a variety of header layouts. Example:', 'metrolo' ) . '</em><br><img src="' . $imagepath . 'header-layout.png">',
	);

	$settings['primary_menuarea'] = array(
		'label'       => __( 'Primary Area (right of logo)', 'metrolo' ),
		'section'     => $section,
		'type'        => 'radio',
		'choices'     => array(
	...
	$settings['secondary_menu_location'] = array(
		'label'       => __( 'Secondary Menu Location', 'metrolo' ),
		'section'     => $section,
		'type'        => 'radio',
		'choices'     => array(
			'top'        => __('Top (above logo)', 'metrolo'),
			'bottom'     => __('Bottom (below logo)', 'metrolo'),
			'none'       => __('Do not display secondary menu', 'metrolo'),
		),
		'default'     => 'bottom',
	);

	$settings['secondary_menu_align'] = array(
		'label'       => __( 'Secondary Menu Alignment', 'metrolo' ),
		'section'     => $section,
		'type'        => 'radio',
		'choices'     => array(
	...
	$settings['logo_image'] = array(
		'label'           => __( 'Upload Logo', 'metrolo' ),
		'section'         => $section,
		'type'            => 'image',
		'priority'        => '165', // Replaced by WP's custom_logo if available // Update in premium if needed
	...
	$settings['logo_image_width'] = array(
		...
		'section'         => $section,
		'type'            => 'text',
		'priority'        => '166', // Keep it with image logo // Update in premium if needed
		'default'         => 200,
	...
	$sections[ $section ] = array(
		...
		'description' => __('Control various color options in the premium version for fonts, backgrounds, contrast, highlight, accent etc.', 'metrolo'),
	);
	...
	$settings['logo_fontface'] = array(
		...
		'section'     => $section,
		'type'        => 'select',
		'priority'    => 345, // Non static options must have a priority
		'choices'     => array(
			...
			'options'     => array(
				...
				'type' => array(
					...
					'choices' => array(
						...
						'image'       => __('Background Image', 'metrolo'),
					),
	...
	$settings['wt_html_slider'] = array(
		'label'       => __( 'Slides', 'metrolo' ),
		'section'     => $section,
		'type'        => 'content',
		'priority'    => 735, // Non static options must have a priority
		'content'     => __( 'Premium version of this theme comes with capability to create <strong>Unlimited slides</strong>.', 'metrolo' ),
	);
	...
	$settings['wt_img_slider'] = array(
		...
		'section'     => $section,
		'type'        => 'content',
		'priority'    => 745, // Non static options must have a priority
		'content'     => __( 'Premium version of this theme comes with capability to create <strong>Unlimited slides</strong>.', 'metrolo' ),
	);
	...
	if ( current_theme_supports( 'woocommerce' ) ) :
		...
		$settings['wooshop_products'] = array(
			...
			'section'     => $section,
			'type'        => 'select',
			'priority'    => '855', // Non static options must have a priority
			'choices'     => $wooproducts,
			'default'     => '12',
...
function hoot_customizer_modify_default_options( $wp_customize ) {
	...
	if ( function_exists( 'the_custom_logo' ) ) {
		...
		$wp_customize->get_control( 'custom_logo' )->priority = 165; // Replaces theme's logo_image // Update in premium if needed
