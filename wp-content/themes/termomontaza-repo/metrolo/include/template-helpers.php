 ...
 * @return void
 */
if ( !function_exists( 'hoot_secondary_menu' ) ):
function hoot_secondary_menu( $location ) {
	$menu_location = hoot_get_mod( 'secondary_menu_location' );
	if ( $location == $menu_location ) {
		?>
		<div <?php hybridextend_attr( 'header-part', 'supplementary', 'contrast-typo' ); ?>>
			<div class="grid">
...
function hoot_update_browser() {
	$notice = '<!--[if lte IE 9]><p class="chromeframe">' .
			  sprintf( __( 'You are using an outdated browser (IE 8 or before). For a better user experience, we recommend %1supgrading your browser today%2s or %3sinstalling Google Chrome Frame%4s', 'metrolo' ), '<a href="http://browsehappy.com/">', '</a>', '<a href="http://www.google.com/chromeframe/?redirect=true">', '</a>' ) .
			  '</p><![endif]-->';
...
function hoot_meta_info_blocks( $args = '', $context = '' ) {
	...
	<div class="entry-byline">
		...
		if ( !empty( $display['author'] ) ) :
			$blocks['author']['label'] = __( 'By:', 'metrolo' );
			ob_start();
		...
		if ( !empty( $display['date'] ) ) :
			$blocks['date']['label'] = __( 'On:', 'metrolo' );
		...
		if ( !empty( $display['cats'] ) ) :
			...
			if ( !empty( $category_list ) ) :
				$blocks['cats']['label'] = __( 'In:', 'metrolo' );
				$blocks['cats']['content'] = $category_list;
			endif;
		endif;

		if ( !empty( $display['tags'] ) && get_the_tags() ) :
			$blocks['tags']['label'] = __( 'Tagged:', 'metrolo' );
			$blocks['tags']['content'] = ( ! get_the_tags() ) ? __( 'No Tags', 'metrolo' ) : get_the_tag_list( '', ', ', '' );
		endif;

		if ( !empty( $display['comments'] ) && comments_open() ) :
			$blocks['comments']['label'] = __( 'With:', 'metrolo' );
			ob_start();
			comments_popup_link(__( '0 Comments', 'metrolo' ),
								__( '1 Comment', 'metrolo' ),
								__( '% Comments', 'metrolo' ), 'comments-link', '' );
			$blocks['comments']['content'] = ob_get_clean();
		endif;

		if ( $edit_link = get_edit_post_link() ) :
			$blocks['editlink']['label'] = '';
			$blocks['editlink']['content'] = '<a href="' . $edit_link . '">' . __( 'Edit This', 'metrolo' ) . '</a>';
		endif;
